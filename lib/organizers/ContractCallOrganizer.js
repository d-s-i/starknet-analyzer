...
class ContractCallOrganizer {
    ...
    */
    static getFullContractAbi(contractAddress, provider) {
        return __awaiter(this, void 0, void 0, function* () {
            let { functions, structs, events } = yield this.organizeContractAbiFromContractAddress(contractAddress, provider);
            ...
            if (getImplementationIndex !== -1) {
                const { result: [implementationClassHash] } = yield provider.callContract({
                    contractAddress,
                    entrypoint: implementationEntryPoints[getImplementationIndex]
                });
                try {
                    const { functions: implementationFunctions, structs: implementationStructs, events: implementationEvents } = yield this.organizeContractAbiFromClassHash(implementationClassHash, provider);
    ...
    static organizeContractAbiFromContractAddress(contractAddress, provider) {
        return __awaiter(this, void 0, void 0, function* () {
            // putting defaultProvider by default bc pathfinder nodes doesn't return abis
            const { abi } = yield starknet_1.defaultProvider.getClassAt(contractAddress, "latest");
            if (!abi) {
    ...
    static organizeContractAbiFromClassHash(classHash, provider) {
        return __awaiter(this, void 0, void 0, function* () {
            // replace provider.getClass(classHash) until it's implemented by starknetjs + pathfinder doesn't return abis
            const { abi } = yield fetch(`http://alpha4.starknet.io/feeder_gateway/get_class_by_hash?classHash=${classHash}`).then(res => res.json());
            if (!abi) {
    ...
    initialize(provider) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { events, functions, structs } = yield ContractCallOrganizer.getFullContractAbi(this.address, _provider);
            this._structs = structs;
            this._functions = functions;
            this._events = events;
    ...
    callViewFn(entrypoint, calldata, provider) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            //     rawResBN
            // ) as any;
            const { subcalldata } = this.organizeCalldata((0, helpers_1.getFullSelectorFromName)(entrypoint), rawResBN);
            return subcalldata;
        });
    }
    organizeCalldata(functionSelector, fullCalldataValues, startIndex) {
